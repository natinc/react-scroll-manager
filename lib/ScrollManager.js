...
function (_React$Component) {
  ...
  function ScrollManager(props) {
    ...
    _this._unlisten = history.listen(function (location, action) {
      ...
      var locationCount = Math.max(0, history.length - _this._historyStart - (action !== "PUSH" ? 1 : 0));
      debug("delete?", {
        _locationsLength: _this._locations.length,
        historyLength: history.length - _this._historyStart,
      ...
      while (_this._locations.length > locationCount) {
        ...
        debug("deletingg ".concat(_key), _this._positions);
      }
  ...
  _createClass(ScrollManager, [{
    ...
    value: function _savePositions() {
      debug("SAVE POSITIONS"); // use pageXOffset instead of scrollX for IE compatibility
    ...
    value: function _savePosition(scrollKey, position) {
      if (!(scrollKey in this._deferredNodes)) {
        ...
        loc[scrollKey] = position; // track the new location key in our array of locations

        this._locations.push(this._locationKey);
      ...
      debug("save", this._locationKey, scrollKey, position, this._positions);
    }
  }, {
    key: "_loadPosition",
    ...
    value: function _restoreNode(scrollKey) {
      ...
      debug("restore", this._locationKey, scrollKey, scrollLeft, scrollTop, this._positions);
    ...
    value: function _restoreWindow() {
      ...
      debug("restore_window", this._locationKey, scrollKey, scrollX, scrollY);
