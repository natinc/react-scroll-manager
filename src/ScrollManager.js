...
export class ScrollManager extends React.Component {
  constructor(props) {
    ...
    this._unlisten = history.listen((location, action) => {
      ...
      const locationCount = Math.max(
        0,
        history.length - this._historyStart - (action !== "PUSH" ? 1 : 0)
      );
      debug("delete?", {
        _locationsLength: this._locations.length,
        historyLength: history.length - this._historyStart,
        locationCount
      });
      while (this._locations.length > locationCount) {
        const key = this._locations.pop();
        delete this._positions[key];
        debug(`deletingg ${key}`, this._positions);
      }
  ...
  _restoreNode(scrollKey) {
    ...
    if (!attemptScroll()) {
      const failedScroll = () => {
        ...
        debug(
          `Could not scroll ${scrollKey} to (${scrollLeft}, ${scrollTop})` +
      ...
      if (timeout) {
        ...
        (this._deferredNodes[scrollKey] = timedMutationObserver(
          attemptScroll,
          timeout,
          node
        ))
  ...
  _restoreWindow() {
    ...
    if (!attemptScroll()) {
      const failedScroll = () => {
        debug(
          `Could not scroll ${scrollKey} to (${scrollX}, ${scrollY})` +
